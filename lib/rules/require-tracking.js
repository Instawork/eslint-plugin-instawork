...
var meta = {
  docs: {
    description: 'Connected components must have tracking code, in order to prevent either of the following ' + 'scenarios: (1) Forgetting to consider tracking, (2) forgetting to hook up tracking, ' + 'resulting in an error (https://sentry.io/instawork/applicant-app/issues/409690489)'
  }
};
  ...
  return {
    Program: function Program() {
      ...
      if (!source.match(/withtracking/i)) {
        var message = 'connected components must have tracking code';
        context.report({ message: message, loc: BEGINNING_OF_FILE });
      }
    }
  };
};

export { create: create, meta: meta };